...
/**
 * Clase que expone servicios REST con ruta base: http://"ip o nombre de host":8080/RotondAndes/rest/equivalenciasProducto/...
 * @author Juan Carreño
 */
@Path("equivalenciasProducto")
public class EquivalenciaProductoServices {
	 ...
	 * <b>URL: </b> http://"ip o nombre de host":8080/RotondAndes/rest/equivalenciasProducto
	...
	public Response getEquivalencias() {
		...
		try {
			equivalencias = tm.darEquivalenciasProducto();
		} catch (Exception e) {
     ...
     * <b>URL: </b> http://"ip o nombre de host":8080/RotondAndes/rest/equivalenciasProducto/nombre/nombre?nombre=<<nombre>>" para la busqueda"
	...
	public Response getEquivalenciaName( @PathParam("nombre") String name) {
		...
		try {
			...
			equivalencias = tm.buscarEquivalenciasProductoPorName(name);
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
		return Response.status(200).entity(equivalencias).build();
     ...
     * <b>URL: </b> http://"ip o nombre de host":8080/RotondAndes/rest/equivalenciasProducto/equivalencia
	...
	@Produces(MediaType.APPLICATION_JSON)
	public Response addEquivalencia(@PathParam( "nombre" ) String nombreRestaurante,EquivalenciaProducto equivalencia) {
		RotondAndesTm tm = new RotondAndesTm(getPath());
		try {
			...
			tm.addEquivalenciaProducto(equivalencia);
			}
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response updateEquivalencia(EquivalenciaProducto equivalencia) {
		RotondAndesTm tm = new RotondAndesTm(getPath());
		try {
			tm.updateEquivalenciaProducto(equivalencia);
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
		return Response.status(200).entity(equivalencia).build();
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response deleteEquivalencia(EquivalenciaProducto equivalencia) {
		RotondAndesTm tm = new RotondAndesTm(getPath());
		try {
			tm.deleteEquivalenciaProducto(equivalencia);
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
		return Response.status(200).entity(equivalencia).build();
